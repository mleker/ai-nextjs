...
const parser = StructuredOutputParser.fromZodSchema(
  z.object({
    ...
    mood: z
      .string()
      .describe('the mood of the person who wrote the journal entry in general terms.'),
    subject: z
      .string()
...
const getPrompt = async (content) => {
  ...
  const prompt = new PromptTemplate({
    template: 'Analyze the following journal entry. Follow the instrusctions and format your response to match the format instructions, no matter what! \n{format_instructions}\n{entry}',
...
export const qa = async (question, entries) => {
  ...
  console.log('ewlwvantDocs------------', relevantDocs)
